...
import { sendRegisterResponse, registerFail } from 'src/service/webauthn'
...
const Register = () => {
  ...
  const onSubmit = data => {
    axios
      .post('http://localhost:8080/webauthn/register', data, {
      ...
      .then(newCred => {
        return sendRegisterResponse(newCred)
      })
      .then(response => {
      ...
      .catch(async e => {
        if (e.response?.data === 'User already exists') {
          ...
          await registerFail(data)
          axios
            .post('http://localhost:8080/webauthn/registerAgain', data, {
              withCredentials: true,
              headers: {
                'Content-Type': 'application/json'
              }
            ...
            .catch(async e => {
              await registerFail(data)
              alert('User clicked cancel, or the registration ceremony timed out')
            })
        }
      })
  }
