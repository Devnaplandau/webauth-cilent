...
import { confirmOTP, sendWebAuthnResponse } from 'src/service/webauthn'
...
const ResetPasswordV1 = () => {
  ...
  const onSubmit = data => {
    const { otp } = data
    confirmOTP({ otp, email: email.index })
      .then(async response => {
        let attResp
      ...
      .then(response => {
        return sendWebAuthnResponse(response)
      })
      .then(response => {
